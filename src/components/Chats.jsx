...
const Chats = ({ bot, tone, history, onSendMessage, isIncognito, setIsIncognito, chatSession }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const responseText = await sendMessage(chatSession, userMessage);
      onSendMessage({ text: responseText, sender: 'bot' });
    } catch (error) {
      console.error('Failed to get response:', error);
      onSendMessage({
        text: `*${bot.name} looks confused* ...Something went wrong. Try again?`,
        sender: 'bot'
      });
    } finally {
      setIsLoading(false);
      ...
      <div className="flex-1 overflow-y-auto py-8 md:py-12 flex flex-col gap-6 custom-scrollbar pr-2 md:pr-4">
        ...
        {isLoading && (
          <div className="flex justify-start animate-in fade-in slide-in-from-bottom-2">
            <div className="max-w-[85%] md:max-w-[65%] p-4 px-7 rounded-[30px] rounded-tl-none border-2 border-retro-red bg-glass-dark backdrop-blur-lg shadow-2xl">
              <div className="flex items-center gap-2">
                <span className="text-white/50 text-sm italic">{bot.name} is thinking</span>
                <div className="flex gap-1">
                  <div className="w-2 h-2 bg-retro-gold rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                  <div className="w-2 h-2 bg-retro-gold rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                  <div className="w-2 h-2 bg-retro-gold rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="pb-6 md:pb-12">
        <div className="relative flex items-center gap-4 bg-glass-dark backdrop-blur-2xl border-2 border-retro-red rounded-[35px] p-4 md:p-5 px-8 shadow-[0_0_30px_rgba(133,24,24,0.3)]">
          ...
          <button
            onClick={handleSend}
            disabled={isLoading}
            className={`group w-12 h-12 bg-black rounded-full flex items-center justify-center text-retro-red border-2 border-retro-red hover:bg-retro-red hover:text-white hover:border-retro-gold transition-all cursor-pointer shadow-lg active:scale-90 ${isLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
