...
const SideBar = ({ isOpen, setIsOpen, bots, activeBot, setActiveBot, selectedTone, setSelectedTone, isIncognito, toggleIncognito, clearChat }) => {
  ...
  return (
    <aside className={`
      fixed inset-y-0 left-0 z-40 w-72 md:w-80 p-6 flex flex-col gap-6 bg-glass-dark backdrop-blur-xl border-r-2 md:border-2 border-retro-red md:rounded-[40px] text-white shadow-2xl transition-all duration-500 animate-pop-up
      md:relative md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}
    `}>
      <Link to="/" className="flex items-center gap-2 text-white/70 hover:text-retro-gold transition-colors group mb-2">
